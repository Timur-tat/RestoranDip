...
@PreAuthorize("hasRole('ADMIN')")
    class AdminController extends CurrentUserSecurity {
    ...
    public void saveAdmin(@RequestBody AdminDTO adminDTO) {
        log.info("Запрос на создание нового Amdin: {}, ", getCurrentUsername());
    ...
    public AdminDTO findById(@PathVariable long id) throws NotFoundException {
        log.info("Запрос на получение Admin по ID: {}, {}", id, getCurrentUsername());
    ...
    public Admin saveWaitersToAdmin(@RequestParam List<Long> waitersId, @PathVariable Long adminId ) {
        log.info("Запрос на получение прикрепление Waiter к Admin: {}, {}",waitersId,adminId, getCurrentUsername());
    ...
    public Admin saveChefToAdmin(@RequestParam List<Long> chefsId, @RequestParam Long adminId ) {
        log.info("Запрос на получение прикрепление Chef к Admin: {}, {}",chefsId,adminId, getCurrentUsername());
